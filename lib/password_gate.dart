...
class _PasswordGateState extends State<PasswordGate> {
  ...
  Future<void> _launchWhatsApp() async {
    try {
      final phone = dotenv.get('PRIMARY_SUPPORT', fallback: '');
      final message = dotenv.get('DEFAULT_MESSAGE', fallback: 'Need assistance');
      
      if (phone.isEmpty) {
        throw 'Support number not configured';
      }
  ...
  Future<void> _validateAccess() async {
    ...
    try {
      ...
      if (mounted) {
        setState(() => _error = 'System error. Please try again');
      }
      debugPrint('Access validation error: $e');
    } finally {
      if (mounted) {
